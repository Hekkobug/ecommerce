...
const DetailProduct = ({ isQuickView, data,location,dispatch,navigate }) => {
  ...
  useEffect(() => {
    if (varriant) {
      const selectedVarriant = product?.varriants?.find((el) => el.sku === varriant);
      setCurrentProduct({
  ...
  const handleAddToCart = async () => {
    ...
    const response = await apiUpdateCart({
      ...
      thumbnail: currentProduct.thumb || product.thumb,  // Đảm bảo thumbnail được truyền đúng cách
      ...
      >
        ...
        >
          ...
          <ul className="list-disc text-sm text-pink-500 pl-4">
            ...
            {product?.description?.length === 1 && (
              <div
                className="text-sm mb-8 line-clamp-[10]"
